# Roadmap do Vestra

## Concluido

- Arquitetura financeira.
- Operacoes.
- Carteira.
- Cotacoes manuais.
- Autocomplete.
- Backup.
- Historico patrimonial.
- Graficos.
- Detalhes de ativos.
- Resumo inteligente.
- Dominio de mercado.
- Provider local.
- Catalogo mestre expandido.
- Busca ranqueada de ativos.
- Cache temporario de mercado.
- Central de dados de mercado.
- Suporte a logos locais.
- Integracao brapi.dev pelo servidor.
- Rotas internas de mercado.
- Cotacoes automaticas em lote.
- Politica manual/automatica de cotacoes.
- Dashboard 2.0 premium.
- Experiencia diaria do Dashboard.
- Timeline patrimonial.
- Recordes locais da jornada.
- Resumo desde a ultima visita.
- Registro local de ultimo acesso ao Dashboard.
- Insights por regras.
- Conquistas discretas.
- Objetivos patrimoniais.
- Objetivos de renda passiva.
- Objetivos manuais.
- Marcos de objetivos na timeline.
- Command Palette com Ctrl+K.
- Area publica Mercado.
- Pesquisa publica de ativos.
- Pagina publica de ativo com indicadores e dividendos disponiveis.
- Asset Experience 2.0 para a pagina publica do ativo.

## Futuro

- Dividendos automaticos.
- Autenticacao.
- Banco de dados.
- Painel administrativo protegido.
- Importacao B3/corretoras.
- Aplicativo movel.
- Inteligencia artificial.
