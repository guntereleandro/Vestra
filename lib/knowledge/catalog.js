...
export const tutorials = [
  ...
  article("tutorial-proventos", "proventos", "tutorials", "Proventos recebidos e eventos esperados", "Entenda a diferença entre anúncio, expectativa e recebimento.", ["Dividendos, JCP e rendimentos em Recebidos são operações confirmadas da fonte ativa.", "Um evento divulgado não prova que a carteira tinha direito nem que o valor foi creditado.", "A visão A receber é separada e não altera patrimônio, snapshots ou performance antes da confirmação.", "Use Atualizar proventos com a fonte Supabase e revise antes de confirmar ou vincular um recebimento.", "Caixa Remunerado, Renda Fixa, amortizações e eventos patrimoniais não são convertidos automaticamente em proventos."], ["proventos", "a receber", "eventos", "conciliação"]),
  article("tutorial-mercado", "mercado", "tutorials", "Mercado", "Pesquise ativos e consulte dados disponíveis do provedor.", ["A busca aceita ticker exato, ticker parcial e nome sem depender de acentos, combinando a listagem da BRAPI com o catálogo local.", "Cotações automáticas passam por rotas internas e o fallback local é identificado quando a origem remota falha.", "A ficha muda conforme a classe: ações, FIIs e ETFs exibem somente dados coerentes.", "Abertura, máxima, mínima, fechamento anterior e volume aparecem quando fornecidos.", "O gráfico oferece 1M e 3M dentro da cobertura do plano atual e usa o fechamento negociado; preço ajustado é preservado separadamente.", "Indicadores avançados dependem dos módulos liberados pelo plano da BRAPI. Ausência permanece indisponível, nunca zero ou atualização fabricada."], ["mercado", "cotações", "indicadores", "histórico"]),
...
export const faq = [article("faq-areas", "areas-atuais", "faq", "Perguntas frequentes", "Respostas rápidas sobre as áreas atuais.", [
  ...
  "Por que o Mercado oferece somente 1M e 3M? Esses são os períodos tratados como suportados pelo plano atual; ranges maiores não são prometidos com base em sandbox.",
  "O preço ajustado aparece no gráfico? O contrato o preserva, mas o gráfico principal usa fechamento negociado e não o interpreta automaticamente como retorno total.",
  "Um dividendo anunciado já conta como recebido? Não. Evento público, expectativa da carteira e operação recebida são fatos separados; somente o recebimento confirmado integra os totais.",
...
export const releases = [article("release-history", "versoes-0-1-a-0-8", "release-notes", "Versões 0.1 a 0.8.2", formatBrandText("Evolução pública do {appName}."), [
  ...
  "CORE-13 — Mercado 2.0: Quote 2.0, classes corretas, histórico 1M/3M, gráfico, capabilities, proveniência, cache, retry e fila compatível com o plano Free.",
  "CORE-14 — Proventos Automáticos: contratos, elegibilidade histórica, A receber e conciliação assistida implementados localmente; o Development foi localizado e a leitura vazia do gate foi atribuída à inicialização transitória do compute. A migration remota permanece pendente.",
], ["releases", "versões", "histórico"] )];

export const glossary = [article("glossary-financial", "glossario-financeiro", "glossary", "Glossário financeiro", formatBrandText("Termos usados no {appName}."), ["Aporte — capital adicionado à carteira.", "Ativo — instrumento financeiro identificado por ticker.", "Auth User — identidade mantida pelo Supabase Auth; não é o Profile de negócio nem contém a carteira.", "Carteira remota — espaço remoto de acesso e permissões.", "Cotação — preço de referência do ativo.", "Close — preço de fechamento negociado no período.", "Adjusted close — fechamento ajustado preservado separadamente; não é assumido automaticamente como retorno total.", "Capability — capacidade declarada de uma fonte, como busca, cotação ou histórico.", "Proveniência — origem, timestamp, disponibilidade, limitação e fallback de um dado.", "Evento de provento — fato público global, ainda sem afirmar direito ou recebimento da carteira.", "Expectativa de provento — valor privado calculado para carteira elegível; não é operação recebida.", "Reconciliação — vínculo auditável entre expectativa e operação confirmada.", "Drawdown — queda desde um pico observado.", "Owner — membro com permissão administrativa sobre a carteira remota.", "Patrimônio — valor atual consolidado das posições.", "Profile — dados públicos mínimos associados ao Auth User.", "Proventos — dividendos, JCP e rendimentos registrados.", "Rentabilidade — variação de desempenho separada dos aportes.", "Sessão — estado temporário de acesso autenticado mantido em cookies seguros.", "Ticker — código de negociação ou identificação do ativo.", "Valorização — componente residual da evolução após fluxos e proventos."], ["glossário", "termos", "mercado", "proventos"] )];
export const changelog = [article("changelog-product", "historico-produto", "changelog", "Histórico do produto", "Registro resumido de mudanças.", ["CORE-14 — Proventos Automáticos implementados localmente sem crédito automático; validação remota permanece pendente.", "CORE-13 — Mercado 2.0 com dados ausentes explícitos, classes por metadata, Quote 2.0, histórico e fila Free.", "CORE-05 — Schema versionado de profiles, carteiras e membros, RLS, criação atômica e repositories remotos; Provider Local financeiro preservado.", "CORE-04 — Autenticação por e-mail e senha, confirmação, recuperação, sessão SSR, callback PKCE e rota Conta protegida; dados financeiros permanecem locais.", "CORE-02 — Contratos assíncronos para dados essenciais, adapters locais, registry, serviços sem alteração de armazenamento.", "Correção crítica de Mercado — conexão HTTPS local, timestamp ISO da BRAPI e cotação básica integrada à resposta do ativo.", "Recuperação funcional — Pesquisa de mercado combinada, mapeamento dos módulos reais da BRAPI e fallback para restrições de plano.", "0.8.2 — Repositório de conhecimento, contrato público, schema editorial e fallback local.", "0.8.1 — Artigos individuais, relacionados, links normalizados e integração com a Command Palette.", "Consulte Release Notes para marcos por versão.", "Mudanças futuras devem atualizar tutorial, FAQ, releases, glossário quando aplicável e metadata.json."], ["changelog", "mudanças", "mercado", "proventos"] )];
