...
function localResolution(reason = "PREFERENCE_LOCAL") {
  return {
    ...
    snapshotsRepository: localPortfolioSnapshotsRepository,
  };
}
...
export async function resolveOperationsDataSource({ loadRemote = true } = {}) {
  ...
  const resolution = {
    ...
    snapshotsRepository: supabasePortfolioSnapshotsRepository,
  };
  if (loadRemote) {
    try {
...
export async function loadRemotePortfolioData(resolution, { allowCache = true } = {}) {
  ...
  const [operations, assetsMaster, quotes, portfolioHistory] = await Promise.all([
    ...
    supabasePortfolioSnapshotsRepository.listByPortfolio(portfolioId),
  ]);
  const updatedAt = operations.reduce(
    (latest, operation) => operation.updatedAt > latest ? operation.updatedAt : latest,
    "",
  );
  const data = { operations, assetsMaster, quotes, portfolioHistory, loadedAt: new Date().toISOString(), updatedAt };
