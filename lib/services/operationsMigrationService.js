...
function comparable(operation) {
  return JSON.stringify({
    ...
    ratioFrom: operation.ratioFrom,
    ratioTo: operation.ratioTo,
    attributedCost: operation.attributedCost,
    targetTicker: operation.targetTicker,
    targetAssetName: operation.targetAssetName,
    targetAssetType: operation.targetAssetType,
    targetQuantity: operation.targetQuantity,
    transferredCost: operation.transferredCost,
...
function validateLocalOperations(operations) {
  ...
  const invalid = operations.flatMap((operation, index) => {
    ...
    const validation = validatePortfolioEvent(item);
    if (!item || !isOperationUuid(item.id) || !validation.valid) {
      return [{ index, id: operation?.id || null, reason: validation.reason || "INVALID_OPERATION" }];
    }
