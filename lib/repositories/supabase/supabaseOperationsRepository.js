...
const COLUMNS = [
  ...
  "trade_date", "quantity", "unit_price", "fees", "income_amount", "notes",
...
function normalizeOne(input, expectedId) {
  ...
  if (!operation || !isOperationUuid(operation.id)) {
    throw repositoryError(REPOSITORY_ERROR_CODES.INVALID_INPUT);
  }
  return operation;
}
...
function fromRow(row) {
  ...
  const [operation] = normalizeOperations([{
    ...
    totalValue: row.income_amount === null ? undefined : Number(row.income_amount),
    notes: row.notes,
  }]);
