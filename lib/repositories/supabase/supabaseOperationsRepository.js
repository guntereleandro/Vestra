import { isCashOperation, isIncomeOperation, isOperationUuid, normalizeOperations, validatePortfolioEvent } from "../../data/operations.js";
...
const COLUMNS = [
  ...
  "trade_date", "quantity", "unit_price", "fees", "income_amount", "cash_amount", "ratio_from", "ratio_to", "attributed_cost", "target_ticker", "target_asset_name", "target_asset_type", "target_quantity", "transferred_cost", "notes",
...
function fromRow(row) {
  ...
  const [operation] = normalizeOperations([{
    ...
    totalValue: row.income_amount != null ? Number(row.income_amount) : row.cash_amount != null ? Number(row.cash_amount) : undefined,
