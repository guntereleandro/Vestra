import { INITIAL_ASSETS, mergeAssetsMaster, normalizeAssetsMaster } from "./assetsMaster.js";
import { migrateOperationIds, normalizeOperations } from "./operations.js";
import { GOAL_MILESTONES_KEY, GOALS_KEY } from "./goals.js";
import { LAST_DASHBOARD_VISIT_KEY } from "./lastDashboardVisit.js";
import { normalizePortfolioHistory, PORTFOLIO_HISTORY_KEY, readPortfolioHistory, upsertPortfolioSnapshot, writePortfolioHistory } from "./portfolioHistory.js";
import { normalizeQuotes } from "./quotes.js";
import { JOURNEY_RECORDS_KEY } from "./journeyRecords.js";
import { appConfig } from "../config/appConfig.js";
import { calculatePositions } from "../engine/portfolio.js";
import { nonNegativeNumber } from "../engine/validations.js";
import { MARKET_CACHE_KEY } from "../market/quoteCache.js";
import { DIAGNOSTIC_PREFERENCES_KEY, normalizeDiagnosticPreferences, readDiagnosticPreferences, validateDiagnosticPreferences } from "./diagnosticPreferences.js";
import { RISK_PROFILE_KEY, normalizeRiskProfile, readRiskProfile, validateRiskProfile } from "./riskProfile.js";

export const STORAGE_KEYS = { operations: "vestra:operations:v1", assetsMaster: "vestra:assetsMaster:v1", quotes: "vestra:assetQuotes:v1", portfolioHistory: PORTFOLIO_HISTORY_KEY, diagnosticPreferences: DIAGNOSTIC_PREFERENCES_KEY, riskProfile: RISK_PROFILE_KEY, journeyRecords: JOURNEY_RECORDS_KEY, lastDashboardVisit: LAST_DASHBOARD_VISIT_KEY, goals: GOALS_KEY, goalMilestones: GOAL_MILESTONES_KEY, marketCache: MARKET_CACHE_KEY, legacyAssets: "vestra:assets:v1", legacyQuotes: "vestra:quotes:v1", migration: "vestra:migration:operations:v1" };
...
export function readLocalData() {
  ...
  let migrated = false;
  const operationIdMigration = migrateOperationIds(parse(rawOperations, []));
  let operations = normalizeOperations(operationIdMigration.operations);
  if (operationIdMigration.migrated) {
  ...
  return { operations, assetsMaster, quotes, portfolioHistory: readPortfolioHistory(), diagnosticPreferences: readDiagnosticPreferences(), riskProfile: readRiskProfile(), migrated };
}
...
export function createBackup() {
  ...
  return { version: BACKUP_VERSION, schemaVersion: BACKUP_VERSION, appVersion: appConfig.appVersion, exportedAt: new Date().toISOString(), operations: data.operations, quotes: data.quotes, assetsMaster: customizedAssets(data.assetsMaster), portfolioHistory: data.portfolioHistory, diagnosticPreferences: data.diagnosticPreferences, riskProfile: data.riskProfile };
}

export function validateBackup(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("INVALID_STRUCTURE");
  if (![1, 2, 3, 4, 5, BACKUP_VERSION].includes(value.version)) throw new Error("UNKNOWN_VERSION");
  ...
  const preferencesValidation = validateDiagnosticPreferences(value.diagnosticPreferences || {});
  if (!preferencesValidation.valid) throw new Error("INVALID_STRUCTURE");
  const riskValidation = validateRiskProfile(value.riskProfile || {});
  if (!riskValidation.valid) throw new Error("INVALID_STRUCTURE");
  return { operations: normalizeOperations(migrateOperationIds(value.operations).operations), assetsMaster, quotes: normalizeQuotes(value.quotes, assetsMaster), portfolioHistory: normalizePortfolioHistory(value.portfolioHistory), diagnosticPreferences: preferencesValidation.value, riskProfile: riskValidation.value };
}
...
export function restoreBackup(value) {
  ...
  localStorage.setItem(STORAGE_KEYS.diagnosticPreferences, JSON.stringify(normalizeDiagnosticPreferences(data.diagnosticPreferences)));
  localStorage.setItem(STORAGE_KEYS.riskProfile, JSON.stringify(normalizeRiskProfile(data.riskProfile)));
