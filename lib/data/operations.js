...
export const PORTFOLIO_EVENT_TYPES = [...OPERATION_TYPES, ...CORPORATE_ACTION_TYPES, ...CASH_OPERATION_TYPES];
...
export function normalizeOperations(value) {
  ...
  return value.flatMap((operation, index) => {
    ...
    const moneyOnly = isIncomeOperation(operationType) || isCashOperation(operationType);
...
export function validatePortfolioEvent(operation) {
  ...
  if (isIncomeOperation(operation.operationType) || isCashOperation(operation.operationType)) return { valid: operation.totalValue > 0, reason: "INVALID_AMOUNT" };
