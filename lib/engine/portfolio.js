...
import { isCashOperation, isIncomeOperation } from "../data/operations.js";
...
function calculateStates(operations) {
  ...
  for (const operation of chronologicalOperations(operations)) {
    ...
    if (operation.operationType === "COMPRA") {
      ...
      if (state.cash) state.held += operation.totalValue;
...
function positionFromState(state, quote, masterAsset) {
  const averagePrice = state.held ? state.invested / state.held : 0;
  const hasMarketQuote = Boolean(quote && Number.isFinite(Number(quote.currentQuote)) && Number(quote.currentQuote) >= 0);
  const hasQuote = state.cash || hasMarketQuote;
  const currentPrice = state.cash ? 1 : hasMarketQuote ? Number(quote.currentQuote) : averagePrice;
  ...
  return {
    ...
    hasQuote,
    quoteUpdatedAt: state.cash ? "" : quote?.updatedAt || "",
    quoteOrigin: state.cash ? "cash-ledger" : quote?.origin || "",
    quoteStale: state.cash ? false : Boolean(quote?.stale),
    manualOverride: state.cash ? false : Boolean(quote?.manualOverride),
