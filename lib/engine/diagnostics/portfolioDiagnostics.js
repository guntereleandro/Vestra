...
function validOperation(item) { return item && typeof item === "object" && typeof item.ticker === "string" && item.ticker && ["COMPRA", "VENDA", "DIVIDENDO", "JCP", "RENDIMENTO"].includes(item.operationType) && Number.isFinite(Number(item.totalValue)) && Number(item.totalValue) >= 0; }
