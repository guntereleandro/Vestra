...
export function calculateBehaviorMetrics(operations = [], generatedAt, nearbyOperationDays = 7) {
  ...
  const buys = valid.filter((item) => item.operationType === "COMPRA"), sales = valid.filter((item) => item.operationType === "VENDA"), income = valid.filter((item) => INCOME_TYPES.has(item.operationType));
...
function validOperation(item) { return item && typeof item.ticker === "string" && item.ticker && ["COMPRA", "VENDA", ...INCOME_TYPES].includes(item.operationType) && /^\d{4}-\d{2}-\d{2}$/.test(item.date || "") && Number.isFinite(Date.parse(`${item.date}T00:00:00Z`)); }
