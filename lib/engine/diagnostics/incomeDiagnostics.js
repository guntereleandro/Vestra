...
const INCOME_TYPES = new Set(["DIVIDENDO", "JCP", "RENDIMENTO"]);
...
export function analyzeIncome(operations, positions, rules, suppliedIncome = []) {
  ...
  const validIncome = [...operations, ...incomeRecords].filter((operation) => INCOME_TYPES.has(operation?.operationType) && operation.ticker && finiteNonNegative(operation.totalValue) && validDate(operation.date));
...
function normalizeIncomeRecord(item) { if (!item || typeof item !== "object") return null; return { ticker: item.ticker, operationType: INCOME_TYPES.has(item.operationType) ? item.operationType : "DIVIDENDO", totalValue: item.totalValue ?? item.value ?? item.amount, date: item.date }; }
