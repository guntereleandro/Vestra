import { isIncomeOperation, isPassiveIncomeOperation } from "../data/operations.js";
...
export function getJourneyRecordCandidates({ operations, positions, totals, portfolioHistory }) {
  ...
  const largestDividend = maxOperation(operations, isPassiveIncomeOperation);
...
function milestoneEvents(operations) {
  ...
  const firstDividend = firstByDate(operations.filter(isPassiveIncomeOperation));
...
export function prepareInsights({ operations, positions, portfolioHistory, visitChanges }) {
  ...
  const hasDividends = operations.some(isPassiveIncomeOperation);
...
export function heroContextMessage({ operations, totals, records, portfolioHistory, visitChanges }) {
  ...
  const recentIncome = operations.some((operation) => isPassiveIncomeOperation(operation) && (Date.now() - dateTime(operation.date)) <= 7 * DAY_MS);
...
export function prepareVisitChanges({ previousVisit, currentSnapshot }) {
  if (!previousVisit?.snapshot) {
    return { lastAccessText: "", hasChanges: false, items: [], emptyTitle: "Primeira visita acompanhada", emptyText: `A partir de agora, o ${brandConfig.appName} mostrará o que mudou desde seu último acesso.` };
  }
...
export function prepareAchievements({ operations, totals, goals = [] }) {
  const achievements = [
    ...
    { key: "first-dividend", title: "Primeiro dividendo", unlocked: operations.some(isPassiveIncomeOperation) },
