...
assert(hook.includes("dataSource.source !== DATA_SOURCE.LOCAL"), "Persistencia local nao esta protegida por fonte.");
...
assert(portfolioService.includes("portfolioHistoryUnavailable: true"), "Historico remoto precisa ser explicitamente indisponivel.");
assert(portfolioService.includes("if (source.source === DATA_SOURCE.SUPABASE)"), "Carga remota precisa de ramo explicito.");
assert(portfolioService.indexOf("portfolioHistoryUnavailable: true") < portfolioService.indexOf("export async function savePortfolioData"), "Carga remota nao esta isolada da gravacao Local.");
