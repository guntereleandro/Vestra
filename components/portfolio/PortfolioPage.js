...
export default function PortfolioPage() {
  ...
  const { positions, totals, operations, assetsMaster, assetQuotes, setAssetQuotes, portfolioHistory, loaded, storageError, dataSource, sourceError, useLocalSource } = data;
  ...
  return <div className="page-container"><DataSourceBadge dataSource={dataSource} sourceError={sourceError} onUseLocal={useLocalSource} />
    ...
    <section className="card mt-6 overflow-hidden rounded-2xl"><div className="flex flex-col gap-3 border-b border-white/[.06] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-xl">Posicoes atuais</h2><p className="mt-1 text-xs text-[#777d78]">{positions.length} {positions.length === 1 ? "ativo consolidado" : "ativos consolidados"}</p></div><div className="flex flex-col gap-2 sm:flex-row"><label className="relative"><span className="sr-only">Buscar ativo</span><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#626762]" size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar ativo..." className="field field-with-prefix-icon py-2.5 text-xs sm:w-44" /></label><select aria-label="Filtrar tipo de ativo" value={type} onChange={(event) => setType(event.target.value)} className="field py-2.5 text-xs sm:w-40"><option>Todos</option>{ASSET_TYPES.map((item) => <option key={item}>{item}</option>)}</select></div></div>{loaded ? <PositionsTable positions={filtered} emptyMessage={positions.length ? "Nenhuma posicao encontrada" : "Sua carteira ainda esta vazia"} onEditQuote={setEditingQuote} /> : <div className="p-12 text-center text-sm text-[#777d78]">Calculando carteira...</div>}</section>
    <p className="mt-4 text-[10px] text-[#555b56]">Cotacoes automaticas dependem da configuracao do provedor. O preco medio e usado apenas como fallback visual.</p>{storageError && !sourceError && <p className="mt-2 text-xs text-rose-400">Nao foi possivel acessar a fonte atual.</p>}{editingQuote && dataSource?.source === "LOCAL" && <QuoteModal position={editingQuote} quote={assetQuotes.find((item) => item.ticker === editingQuote.ticker)} onClose={() => setEditingQuote(null)} onSave={saveQuote} onClear={clearQuote} onUseAutomatic={preferAutomatic} />}
  </div>;
}
