...
export default function MarketDataCenter({ positions = [], setAssetQuotes }) {
  ...
  async function updateQuotes() {
    const result = await refreshQuotes(positions.map((position) => position.ticker), { force: true });
    if (result.quotes?.length && setAssetQuotes) setAssetQuotes((current) => applyAutomaticQuotes(current, result.quotes));
    setStats(cacheStats());
    setSummary({ updated: result.fetched?.length || 0, cached: result.cached?.length || 0, notFound: result.notFound?.length || 0, failed: result.failed?.length || 0, at: new Date().toLocaleString("pt-BR") });
  }

  async function verifyProvider() {
    setChecking(true);
    try { setStatus(await providerStatus({ verify: true })); }
  ...
  return <section className="card mt-6 overflow-hidden rounded-2xl">
    ...
    <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-4 sm:p-6">
      ...
      <MarketCard label="Status" value={notConfigured ? "Não configurado" : status?.connectivity === "online" ? "Online" : status?.connectivity === "unverified" ? "Não verificado" : "Indisponível"} detail={status?.message || "Fallback local ativo."} />
    ...
    <div className="flex flex-col gap-3 border-t border-white/[.06] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><p className="text-xs leading-relaxed text-[#777d78]">A chave da brapi fica somente no servidor. A carteira continua funcionando com cotacoes manuais e dados locais se a API estiver indisponivel.</p><div className="flex flex-col gap-2 sm:flex-row"><button type="button" onClick={verifyProvider} disabled={checking || notConfigured} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-xs font-bold text-[#c5c8c4] disabled:opacity-40"><RefreshCw size={15} />{checking ? "Verificando..." : "Verificar conexão"}</button><button type="button" onClick={updateQuotes} disabled={loading || !positions.length || notConfigured} className="flex items-center justify-center gap-2 rounded-xl border border-[#d9b86c]/25 bg-[#d9b86c]/5 px-4 py-3 text-xs font-bold text-[#d9b86c] disabled:opacity-40"><RefreshCw size={15} />{loading ? "Atualizando..." : "Atualizar cotacoes"}</button><button type="button" onClick={cleanCache} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-xs font-bold text-[#c5c8c4]"><Trash2 size={15} />Limpar cache</button></div></div>
    {summary && <p role="status" className="px-5 pb-3 text-xs text-[#898e89] sm:px-6">Última atualização: {summary.at}. Atualizadas: {summary.updated}. Cache: {summary.cached}. Indisponíveis: {summary.notFound}. Falharam: {summary.failed}.</p>}
