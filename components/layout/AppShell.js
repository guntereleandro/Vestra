...
function Brand() {
  return <Link href="/" className="flex items-center gap-3 text-[#d7d9d5] transition hover:text-white">
    ...
    <div>
      <p className="font-display text-lg leading-none">Vestra</p>
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[16rem_minmax(0,1fr)]">
    ...
    {menuOpen && <div className="fixed inset-0 z-[60] bg-black/75 p-4 backdrop-blur-sm lg:hidden" onMouseDown={(event) => event.target === event.currentTarget && setMenuOpen(false)}>
      <div className="card mx-auto mt-12 max-w-lg rounded-3xl p-5">
        <div className="flex items-center justify-between"><div><p className="eyebrow">Navegação</p><h2 className="font-display mt-1 text-2xl">Explorar Vestra</h2></div><button onClick={() => setMenuOpen(false)} className="icon-button" aria-label="Fechar navegação"><X size={18} /></button></div>
